...
else if ( !preg_match("/{$value1}/i", $_POST['ab']) === true || !preg_match("/{$value2}/i", $_POST['ab']) === true   ){
	
	 echo "<script>
 	
alert('Wrong SQL clause/claises used for this question ');
...
alert('Updating a wrong field or column');
...
else if ( !preg_match("/{$value6}/i", $_POST['ab']) === true   ){
	
	 echo "<script>
 	
alert(Wrong value for RecoveringStatus. the correct RecoveringStatus value is Yes);
